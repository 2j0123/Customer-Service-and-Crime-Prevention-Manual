...
def display_money_demand_question():
    ...
    with col3:
        if st.button('선택', key='scam1_select'):
            select_scam(1)
        st.image(os.path.join(config.IMG_FOLDER, 'family_scam.jpg'), use_column_width=True)
    ...
    with col4:
        if st.button('선택', key='scam2_select'):
            select_scam(2)
        st.image(os.path.join(config.IMG_FOLDER, 'government_scam.jpg'), use_column_width=True)
    ...
    with col5:
        if st.button('선택', key='scam3_select'):
            select_scam(3)
        st.image(os.path.join(config.IMG_FOLDER, 'app_scam.jpg'), use_column_width=True)
...
def display_crime_options():
    ...
    with col6:
        if st.button('보이스피싱 대응 매뉴얼', key='scam_manual'):
            st.image(os.path.join(config.IMG_FOLDER, 'voice1.png'))
            st.image(os.path.join(config.IMG_FOLDER, 'voice2.png'))
    with col7:
...
def display_finance_options():
    ...
    with col9:
        if st.button('현금입금', key='cash_deposit'):
            st.image(os.path.join(config.IMG_FOLDER, 'deposit.png')) 
    with col10:
        if st.button('현금출금', key='cash_withdrawal'):
            st.image(os.path.join(config.IMG_FOLDER, 'withdrawal.png'))
    with col11:
        if st.button('계좌송금', key='account_transfer'):
            st.image(os.path.join(config.IMG_FOLDER, 'remittance.png')) 
    with col12:
        if st.button('거래조회', key='transaction_inquiry'):
            st.image(os.path.join(config.IMG_FOLDER, 'transactional_information.png'))
    with col13:
        if st.button('통장정리', key='update_bankbook'):
            st.image(os.path.join(config.IMG_FOLDER, 'update_bankbook.png')) 
    with col14:
