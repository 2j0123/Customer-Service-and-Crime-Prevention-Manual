...
def display_initial_options():
    ...
    with col1:
        st.button('예', on_click=set_step, args=(1,), key='initial_yes', help='red')
    with col2:
        st.button('아니오', on_click=set_step, args=(4,), key='initial_no', help='green')
...
def display_money_demand_question():
    ...
    with col3:
        if st.button('선택', key='scam1_select'):
            select_scam(1)
        st.image(r'C:\Users\USER\Pictures\Screenshots\family_acquaintance_impersonation.jpg', use_column_width=True)
    ...
    with col4:
        if st.button('선택', key='scam2_select'):
            select_scam(2)
        st.image(r'C:\Users\USER\Pictures\Screenshots\Government_financial_institution_impersonation.jpg', use_column_width=True)
    ...
    with col5:
        if st.button('선택', key='scam3_select'):
            select_scam(3)
        st.image(r'C:\Users\USER\Pictures\Screenshots\induce_app_installation.jpg', use_column_width=True)
...
def display_crime_options():
    ...
    with col6:
        if st.button('보이스피싱 대응 매뉴얼', key='scam_manual'):
            st.image(r'C:\Users\USER\Pictures\Screenshots\voice1.png')
            st.image(r'C:\Users\USER\Pictures\Screenshots\voice2.png')
    with col7:
...
def display_finance_options():
    ...
    with col9:
        if st.button('현금입금', key='cash_deposit'):
            st.image(r'C:\Users\USER\Pictures\Screenshots\deposit.png') 
    with col10:
        if st.button('현금출금', key='cash_withdrawal'):
            st.image(r'C:\Users\USER\Pictures\Screenshots\withdrawal.png')
    with col11:
        if st.button('계좌송금', key='account_transfer'):
            st.image(r'C:\Users\USER\Pictures\Screenshots\remittance.png') 
    with col12:
        if st.button('거래조회', key='transaction_inquiry'):
            st.image(r'C:\Users\USER\Pictures\Screenshots\transactional_information.png')
    with col13:
        if st.button('통장정리', key='update_bankbook'):
            st.image(r'C:\Users\USER\Pictures\Screenshots\update_bankbook.png') 
    with col14:
